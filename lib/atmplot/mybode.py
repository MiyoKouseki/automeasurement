...
class BodePlot(Plot):
    ...
    **kwargs
    ...
    def __init__(self, *filters, **kwargs):
        ...
        coherences = kwargs.pop('coherence', None)
        if coherences:
            self.add_subplot(3, 1, 1)
            self.add_subplot(3, 1, 2, sharex=self.maxes)            
            self.add_subplot(3, 1, 3, sharex=self.maxes)
        else:
            self.add_subplot(2, 1, 1)
            self.add_subplot(2, 1, 2, sharex=self.maxes)            

        # add filters
        for filter_,coherence in zip(filters,coherences):
            if isinstance(filter_, FrequencySeries):
                self.add_frequencyseries(filter_, coherence=coherence,
                                         dB=dB, **kwargs)
            else:
        ...
        if coherence:
            self.caxes.set_ylim(0,1.01)
            self.caxes.set_ylabel('Coherence')            
            self.caxes.set_xlabel('Frequency [Hz]')
        else:
            self.maxes.set_xlabel('Frequency [Hz]')    
    ...
    def add_frequencyseries(self, spectrum, coherence=None, dB=True, power=False, **kwargs):
        ...
        **kwargs
        ...
        if coherence:
            ...
            cline = self.caxes.plot(w, coh, **kwargs)[0]
        return mline, pline
